...
def remap(df):
    """Remaps columns in mappluto dataset to be human-readable."""
    pd.options.mode.chained_assignment = None
    fields = ["areasource", "bsmtcode", "landuse", "ownertype", "bldgclass"]

    if fields[0] in df.columns:
        df[fields[0]].replace(AREASOURCE, regex=False, inplace=True)
    if fields[1] in df.columns:
        df[fields[1]].replace(BSMTCODE, regex=False, inplace=True)
    if fields[2] in df.columns:
        df[fields[2]].replace(LANDUSE, regex=False, inplace=True)
    if fields[3] in df.columns:
        df[fields[3]].replace(OWNERTYPE, regex=False, inplace=True)
    if fields[4] in df.columns:
        df[fields[4]].replace(BLDGCLASS, regex=False, inplace=True)

    pd.options.mode.chained_assignment = 'warn'
    return df


CONFIG_INI_SECTION = {'nycdb': 'nycdb',
                      'qgis': 'nycdb.qgis'}
...
BSMTCODE = {
    "0": "None/No Basement",
    "1": "Above grade full basement – the basement is 75% or more of the area of the first floor and the basement\
                walls are at least 4 feet high on at least two sides",
    "2": "Below grade full basement – the basement is 75% or more of the area of the first floor and the basement\
                walls are fully submerged or are less than 4 feet on at least three sides",
    "3": "Above grade partial basement – the basement is between 25% and 75% of the area of the first floor and\
                the basement walls are at least 4 feet high on at least two sides",
    "4": "Below grade partial basement – the basement is between 25% and 75% of the area of the first floor and\
                the basement walls are fully submerged or are less than 4 feet on at least three sides",
    "5": "Unknown",
}
